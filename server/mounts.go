...
func (s *Server) Mounts() []environment.Mount {
	...
	if cfg.System.Passwd.Enable {
		...
		m = append(m, environment.Mount{
			Source:   filepath.Join(cfg.System.Passwd.Directory, "sudoers"),
			Target:   "/etc/sudoers",
			ReadOnly: true,
		})
		m = append(m, environment.Mount{
			Source:   filepath.Join(cfg.System.Passwd.Directory, "sudo"),
			Target:   "/run/sudo",
			ReadOnly: false,
		})
		m = append(m, environment.Mount{
			Source:   filepath.Join(cfg.System.Passwd.Directory, "hosts"),
			Target:   "/etc/hosts",
			ReadOnly: true,
		})
	}
